...
async function getHistoryMonthly() {
    ...
    try {
        ...
        const getWeatherIcon = (time, cloud) => {
            ...
            if (cloud > 70) {
                return isDaytime ? "https://www.awxcdn.com/adc-assets/images/weathericons/26.svg" : "https://www.awxcdn.com/adc-assets/images/weathericons/38.svg"; // Changed from 27 to 38
            } else if (cloud > 30) {
                return isDaytime ? "https://www.awxcdn.com/adc-assets/images/weathericons/30.svg" : "https://www.awxcdn.com/adc-assets/images/weathericons/29.svg"; // Changed from 28 to 30
            } else {
                return isDaytime ? "https://www.awxcdn.com/adc-assets/images/weathericons/1.svg" : "https://www.awxcdn.com/adc-assets/images/weathericons/33.svg";
            }
        };
        ...
        historyMonthly.forEach(entry => {
            ...
            const dayKey = dateObj.toLocaleDateString('en-US', { weekday: 'short', day: 'numeric', month: 'short' });
                ...
                <div class="day-column" id="day-column-${dayIndex}">
                    ...
                    <div class="day-data" style="position: relative;">
                        <svg class="temperature-line-svg" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 4;">
                            <path class="temp-path" fill="none" stroke="#ff0000" stroke-width="3" d="${generatePathData(entries)}"/>
                        </svg>
            `;
...
function drawTemperatureLines(dayGroups) {
    Object.entries(dayGroups).forEach(([day, entries]) => {
        ...
        entries.forEach((entry, index) => {
            const tempDot = dayColumn.querySelector(`[data-time="${entry.time}"][data-temp="${entry.temperature}"]`);
